...
void two_way_transport_sweep( Params * params, Input * I )
{
	...
	reduction(+ : segments_processed )
	{
		...
		for (long i = 0; i < I->ntracks_2D; i++)
		{
			...
			double ** seg_dist = (float * )malloc( I->z_stacked * sizeof(double *) );
			Source *** seg_src = (float * )malloc( I->z_stacked * sizeof(Source**) );
			int * seg_idx = (float * )malloc( I->z_stacked * sizeof(int) );
			int * seg_size = (float * )malloc( I->z_stacked * sizeof(int) );
